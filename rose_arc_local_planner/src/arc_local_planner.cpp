...
bool ArcLocalPlanner::findBestCommandVelocity(const vector<PoseStamped>& plan, Twist& best_cmd_vel)
{
        ...
        for(int i = 0; i < num_tang_velocities; ++i)
        {
            ...
            for(int j = 0; j < num_rot_velocities; ++j)
            {
                ...
                for(int k = 1; k < num_dts; ++k)
                {
                    ...
                    trajectory_score.trajectory = FCC_.calculatePoseTrajectory(trajectory_score.velocity, stepsize_dts, forward_t + 3.0, 3.0);  //! @todo OH [IMPR]: Let extra forward sim time depend on acceleration.
    ...
    std::vector<TrajectoryScore*> elements;
